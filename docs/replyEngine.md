...
- **轮次纪元（epoch）**：每个房间记一个版本号，**用户每发一条新消息就 +1**；所有在途/排队的回复都带着出生时的 epoch，执行前（以及 API 返回后）核对——对不上说明用户已经又说话了，**直接作废**（不发、不接龙、收掉"正在输入"）。这防止"上一轮还在排队的回复叠到新一轮上"，否则会出现两轮叠加突破 `MAX_AI_TURNS`、以及旧轮幽灵回复答非所问。已落库的旧回复不受影响（覆水难收）。
...
<人设 systemPrompt>
[场景设定]: <剧本>
[群聊成员]: 除了你，群里还有 林林、小帅（说干就干的工程师）、小芙。这是一个多人群聊…
[用户]: 真人用户叫"林林"。林林的背景: …
...
**成员互相认识（公开简介）**：成员名单里每位 AI 会带上它的 `publicIntro`（人格编辑器里的「公开简介」，格式 `名字（简介）`）。这样大家开局就知道彼此"是谁"，不必从聊天记录里现学；而 systemPrompt 里的底牌（暗中动机等）**不会**贴给别人——藏得住戏。没填简介就只列名字。
